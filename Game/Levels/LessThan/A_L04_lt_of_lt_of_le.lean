...
Introduction"Before we prove that `<` is transitive, we show a stronger result.
It suffices to have proofs for both `x < y` and `y ≤ z` to generate a proof of `x < z`.
That is your task in this level.  Remember, your goal in this level is to generate/discover
a number `n` that is morally `c - succ a`."
...
Statement lt_of_lt_of_le (a b c: ℕ) : a < b → b ≤ c → a < c := by
  ...
  Hint"You can take it from here.  If subtraction was defined,  what number is `{c} - succ {a}` be?"
  use (n + m)
